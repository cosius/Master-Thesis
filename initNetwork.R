...
#cm0: Adjacency matrix, need as input
########################nope nope   #pL: liste med med cliques*
#Names: vektor med variabelnavne
#m: Mean of the variables
#s: Standard deviation


#P: temporary Placeholder for various information
#Hvert element i pot er en liste med
#v-vertices: vertices without separators in a clique
#vertices_Sorted: All vertices sorted in perceft order
#pa-parents: navne på nabovariable 
#pr: lambda (Precision) parameter
#ga-gamma: gamma vektor, som har navneattribut pa
#de-delta: delta parametre
#k-kappa: kontribution to the kappa parameter for each vertice
#m:mean
#s:standard derivative
#pots: list with information on potentials and in order corresponding to the perfect elimination sequence.
#ii: temp holder

#x: observationsvektor - skal have navneattribut Names
...
initNetwork <- function(cm0){ #takes and Adjacency matrix as input ???
    ...
    numberOfCliques <- length(OO$cl) # number of cliques ##L0
    if(numberOfCliques>0) for(i in numberOfCliques:1){ #
        clique_I <- OO$cl[[i]] #clique i ##v
        sep <- OO$se[[i]] #separators for i (vertices in clique_I that are also in parent cliques) ##pa
    ...
    for(i in 1:numberOfCliques){ #for each clique
        ...
        for(j in 1:NoV){ #for each vertice in the clique_I1
            temp_Vert <- list(vertex=P$vertices[j]) #takes out vertex j
            temp_Vert$parents <- if(j<NoV) c(P$vertices[(j+1):NoV],P$separators) else P$separators #add all the parents of that vertice to list parents
            ...
            temp_Vert$pr <- 0 # placeholder for Lambda(precision) for vertice j 
            names(temp_Vert$pr) <- temp_Vert$vertex #give lambda j the name of vertice j
            temp_Vert$delta <- 0 # placeholder for delta for vertice j 
            names(temp_Vert$delta) <- temp_Vert$vertex #give lambda j the name of vertice j
    ...
    #delta <- rep(0,length(pList)) #  a 0 for each vertice
    #names(delta) <- NAM #each 0 in delta is associated with a vertice.    #colnames(dataVertices) --- removed
    ...
    for(i in 1:length(pList)){ # for each vertex
        ...
        } else pr <- (dimData-1)/sum((dataVertices[,P$vertex])^2) # lambda.update if no parents ###pr <- 1/var(dataVertices[,P$vertice]) 
